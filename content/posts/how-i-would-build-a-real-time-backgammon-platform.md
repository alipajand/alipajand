...
Shared primitives — [Tailwind](https://tailwindcss.com/docs/installation/using-vite), [Radix](https://www.radix-ui.com/primitives/docs/overview/introduction), optionally [shadcn/ui](https://ui.shadcn.com/docs) — cover the app chrome (buttons, dialogs, tabs, toasts). Game-specific components (board, checker, dice, doubling cube, clock, move list) are their own layer.
...
Most of the choices above exist so the expensive work stays bounded as the platform grows. It helps to name the variables: `U` concurrent users, `M` live matches, `N` moves in a single game, `L` rows in a leaderboard or history, `V` rows actually visible on screen.

**Game logic is constant-bounded.** A backgammon board is fixed — 24 points, a bar, an off-tray, a few checkers to consider per roll, at most four sub-moves on doubles. Enumerating legal moves for a roll is bounded by a small constant, not by `U` or `M`; it is effectively `O(1)` per turn. That is the whole reason the engine can be a pure, synchronous domain layer: validating or previewing a move never gets more expensive as the product grows.

**Matches are independent, so the system scales horizontally.** Nothing on the gameplay hot path is shared mutable global state. Each match is an isolated room keyed by `gameId`, so total work is `O(M)` spread across as many servers as you want — adding users adds independent `O(1)` rooms instead of contending on one structure. That "embarrassingly parallel" property is what lets the platform scale by sharding rather than rewriting.

**Reconnect is cheap because state is versioned.** Drift detection is an integer compare, `O(1)`. Resync is either one full snapshot (game state is constant-size) or a replay of only the missed events, `O(Δ)` with `Δ` small. The client never re-derives the whole world to recover; it asks how far behind its version is and catches up by the gap.

**Replay is linear in a small `N`.** Folding the event log to derive board state is `O(N)`, and `N` — moves in one game — is in the tens. Jumping to an arbitrary position is also `O(N)`; if scrubbing ever feels heavy, periodic position snapshots make it `O(N / k)` from the nearest checkpoint. This is ordinary event sourcing ([Fowler's write-up](https://martinfowler.com/eaaDev/EventSourcing.html)), and it is why replay is nearly free once live play is correct.

**The UI keeps per-tick work off the `O(total)` path.** A clock tick updates an isolated subtree, `O(1)`, not the whole component tree. Leaderboards and history virtualize, so the DOM holds `O(V)` rows regardless of `L`, and fetching stays `O(page)` through paginated queries. The board is a bounded set of SVG nodes, so it never drifts toward the thousands-of-nodes range where SVG degrades. Entitlement checks read a precomputed boolean rather than scanning plan rules — also `O(1)`.

The summary I would give a team: the only things that grow with usage are the number of independent matches and the length of a few lists, and both have clean answers — shard the matches, window the lists. Everything on the hot path of a single match is bounded by the rules of backgammon, not by how popular the product gets.
