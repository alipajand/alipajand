...
export const useHiringFitReveal = (): {
  ...
  useEffect(() => {
    ...
    const badges = Array.from(section.querySelectorAll<HTMLElement>("[data-hiring-badge]"));
    const cards = Array.from(section.querySelectorAll<HTMLElement>("[data-hiring-card]"));
