...
export function ProofStripMetric({ value, label }: ProofStripMetricProps) {
  const { ref: valueRef, hasParsed } = useCountUp(value);

  return (
    <div data-metric className="min-w-0" data-reveal>
      <dt
        ref={hasParsed ? (valueRef as React.Ref<HTMLElement>) : undefined}
        className="font-display font-semibold text-foreground text-base sm:text-lg leading-tight tracking-tight"
      >
        {value}
      </dt>
